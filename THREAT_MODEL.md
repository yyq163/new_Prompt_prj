...
- RAGFlow SSRF and credential exfiltration: RAGFlow endpoint is server-configured only, rejects unsafe schemes, userinfo, localhost, loopback, private, link-local, multicast, reserved, IPv4-mapped IPv6, and unsafe DNS results. Redirects are not automatically followed, and Authorization is sent only to the approved origin.
