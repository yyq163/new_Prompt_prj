...
- `screenshots_tracked=false`: count must be `0`
- `screenshots_policy=local_only_not_tracked`: count must be `0`
- `local-only` used for screenshot strategy: count must be `0`
- `not tracked` used for screenshot strategy: count must be `0`
...
|---|---|---|---|
| `screenshots_tracked=false` | 0 | 0 | PASS |
| `screenshots_policy=local_only_not_tracked` | 0 | 0 | PASS |
| `local-only` used for screenshot strategy | 0 | **4** | See violations below |
| `not tracked` used for screenshot strategy | 0 | **4** | See violations below |

Violations (phrases applied to screenshot/browser-capture policy, not to trace/network artifacts):

- `.codex-agent-team/reports/provider-base64-receive-and-browser-rerun-20260615.md` lines 44, 53: `Screenshot: local-only, not tracked on main01.` (2 occurrences)
- `.codex-agent-team/reports/final-integrator-main01-20260615.md` line 16: `Screenshot policy at the time: local-only, not tracked.`
- `.codex-agent-team/reports/final-integrator-main01-post-push-template-20260615.md` line 81: `Screenshot/browser-capture policy: untracked / ignored / local-only.`
- `.codex-agent-team/reports/browser-qa-edits-20260615.md` line 33: `No screenshots or browser captures are tracked.`
...
- `evidence/visual-e2e-report.md` line 7: `Trace files and network captures remain local-only and are not tracked.`
- `.codex-agent-team/reports/branch-auditor-main01-20260615.md` line 9: merge-based branch kept `local-only`.
...
2. **Stale policy wording in reports**
   - `.codex-agent-team/reports/provider-base64-receive-and-browser-rerun-20260615.md` states screenshots are `local-only, not tracked on main01`.
   - `.codex-agent-team/reports/final-integrator-main01-20260615.md` states the screenshot policy was `local-only, not tracked`.
   - `.codex-agent-team/reports/final-integrator-main01-post-push-template-20260615.md` lists the default template policy as `untracked / ignored / local-only`.
...
3. **Historical reports retained as audit trail**
   - `.codex-agent-team/reports/final-integrator-main01-post-push-20260615.md`, `.codex-agent-team/reports/security-reviewer-main01-repair-20260615.md`, and `.codex-agent-team/reports/evidence-auditor-main01-repair-20260615.md` are marked HISTORICAL and describe the prior `local_only_not_tracked` policy in the context of the repair cycle. They are acceptable as audit trail but contribute to the overall count of old-policy references.
...
1. **Add screenshot files to the Git index** to satisfy `screenshots_tracked=true`:
   ...
   ```
2. **Archive or update stale reports** that still describe screenshots as `local-only`, `not tracked`, `untracked`, or `ignored`. If retained as audit trail, ensure each file begins with an explicit `HISTORICAL` marker and does not present the old policy as current.
...
- Branch `main01` matches `origin/main01` at `3a5124085176dc0372fd4fb76c38792b15573eb7`.
